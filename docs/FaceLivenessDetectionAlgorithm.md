...
本活体检测系统采用**两阶段检测方案**，通过人脸运动检测和照片攻击检测相结合的方式，有效防范照片、视频等欺骗攻击。系统要求每帧图像中必有人脸且人脸占比足够大，通过分析人脸关键点的几何变化来判断是否为活体。
...
3. **照片检测**：若`isMoving = true`，执行照片攻击检测
4. **最终判定**：结合两阶段结果输出最终判断

### 输出结果

```typescript
{
  isLive: boolean,                    // 是否为活体
  firstStagePassed: boolean,          // 第一阶段（运动检测）是否通过
  secondStagePassed: boolean,         // 第二阶段（照片检测）是否通过
  firstStageDetails: {                // 运动检测详细信息
    isMoving: boolean,
    movementConfidence: number,
    currentMovement: number,
    // ... 其他详细指标
  },
  secondStageDetails: {               // 照片检测详细信息
    isPhoto: boolean,
...
- **第二阶段拦截**：屏幕显示本质是2D投影，缺乏3D透视变化 → 照片特征明显 → `isPhoto = true`

#### 真实人脸
- **第一阶段通过**：自然头部运动 → `isMoving = true`
- **第二阶段通过**：具备真实3D几何特征 → `isPhoto = false` → 判定为活体
...
### 联合调参要点
- 两阶段参数需协调调整，避免某阶段过于敏感导致整体性能下降
- 应充分测试联合效果，而非单独优化各阶段参数